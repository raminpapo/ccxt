# Documentation: ts/src/test/static/currencies/cex.json

## File Metadata

- **Path**: `ts/src/test/static/currencies/cex.json`
- **Size**: 30,300 bytes
- **Lines**: 986
- **Type**: JSON
- **Extension**: .json


## Original Source Code

```json
{
    "BTC": {
        "info": {
            "currency": "BTC",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "8",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "Bitcoin",
            "blockchains": {
                "bitcoin": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.0001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.002",
                    "withdrawalFee": "0.0005",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "2"
                }
            }
        },
        "id": "BTC",
        "numericId": null,
        "code": "BTC",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 0.0005,
        "fees": {},
        "networks": {
            "BTC": {
                "id": "bitcoin",
                "network": "BTC",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 0.0005,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 0.0001,
                        "max": null
                    },
                    "withdraw": {
                        "min": 0.002,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.0001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.002",
                    "withdrawalFee": "0.0005",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "2"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 0.002,
                "max": null
            },
            "deposit": {
                "min": 0.0001,
                "max": null
            }
        }
    },
    "USDT": {
        "info": {
            "currency": "USDT",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "6",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "Tether",
            "blockchains": {
                "ethereum": {
                    "type": "ERC20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "20",
                    "withdrawalFee": "10",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                },
                "tron": {
                    "type": "TRC20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "5",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "21"
                },
                "solana": {
                    "type": "SPL",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                },
                "binancesmartchain": {
                    "type": "BEP20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "2",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                }
            }
        },
        "id": "USDT",
        "numericId": null,
        "code": "USDT",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 1,
        "fees": {},
        "networks": {
            "ethereum": {
                "id": "ethereum",
                "network": "ethereum",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 10,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 20,
                        "max": null
                    }
                },
                "info": {
                    "type": "ERC20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "20",
                    "withdrawalFee": "10",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                }
            },
            "TRC20": {
                "id": "tron",
                "network": "TRC20",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 5,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 10,
                        "max": null
                    }
                },
                "info": {
                    "type": "TRC20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "5",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "21"
                }
            },
            "SOLANA": {
                "id": "solana",
                "network": "SOLANA",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 1,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 10,
                        "max": null
                    }
                },
                "info": {
                    "type": "SPL",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            },
            "BSC20": {
                "id": "binancesmartchain",
                "network": "BSC20",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 2,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 10,
                        "max": null
                    }
                },
                "info": {
                    "type": "BEP20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "2",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 10,
                "max": null
            },
            "deposit": {
                "min": 5,
                "max": null
            }
        }
    },
    "EUR": {
        "info": {
            "currency": "EUR",
            "fiat": true,
            "precision": "8",
            "walletPrecision": "2",
            "walletDeposit": true,
            "walletWithdrawal": true
        },
        "id": "EUR",
        "numericId": null,
        "code": "EUR",
        "precision": 1e-8,
        "type": "fiat",
        "name": null,
        "active": null,
        "deposit": true,
        "withdraw": true,
        "fee": null,
        "fees": {},
        "networks": {},
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": null,
                "max": null
            }
        }
    },
    "USD": {
        "info": {
            "currency": "USD",
            "fiat": true,
            "precision": "8",
            "walletPrecision": "2",
            "walletDeposit": true,
            "walletWithdrawal": true
        },
        "id": "USD",
        "numericId": null,
        "code": "USD",
        "precision": 1e-8,
        "type": "fiat",
        "name": null,
        "active": null,
        "deposit": true,
        "withdraw": true,
        "fee": null,
        "fees": {},
        "networks": {},
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": null,
                "max": null
            }
        }
    },
    "LTC": {
        "info": {
            "currency": "LTC",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "8",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "Litecoin",
            "blockchains": {
                "litecoin": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.03",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.05",
                    "withdrawalFee": "0.02",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "3"
                }
            }
        },
        "id": "LTC",
        "numericId": null,
        "code": "LTC",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 0.02,
        "fees": {},
        "networks": {
            "LTC": {
                "id": "litecoin",
                "network": "LTC",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 0.02,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 0.03,
                        "max": null
                    },
                    "withdraw": {
                        "min": 0.05,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.03",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.05",
                    "withdrawalFee": "0.02",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "3"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 0.05,
                "max": null
            },
            "deposit": {
                "min": 0.03,
                "max": null
            }
        }
    },
    "ETH": {
        "info": {
            "currency": "ETH",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "6",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "Ethereum",
            "blockchains": {
                "ethereum": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.02",
                    "withdrawalFee": "0.006",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                },
                "optimism": {
                    "type": "coin",
                    "deposit": "disabled",
                    "minDeposit": null,
                    "withdrawal": "disabled",
                    "minWithdrawal": null,
                    "withdrawalFee": null,
                    "withdrawalFeePercent": null,
                    "depositConfirmations": "32"
                }
            }
        },
        "id": "ETH",
        "numericId": null,
        "code": "ETH",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 0.006,
        "fees": {},
        "networks": {
            "ethereum": {
                "id": "ethereum",
                "network": "ethereum",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 0.006,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 0.001,
                        "max": null
                    },
                    "withdraw": {
                        "min": 0.02,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.02",
                    "withdrawalFee": "0.006",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                }
            },
            "OPT": {
                "id": "optimism",
                "network": "OPT",
                "margin": null,
                "deposit": false,
                "withdraw": false,
                "active": false,
                "fee": null,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": null,
                        "max": null
                    },
                    "withdraw": {
                        "min": null,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "disabled",
                    "minDeposit": null,
                    "withdrawal": "disabled",
                    "minWithdrawal": null,
                    "withdrawalFee": null,
                    "withdrawalFeePercent": null,
                    "depositConfirmations": "32"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 0.02,
                "max": null
            },
            "deposit": {
                "min": 0.001,
                "max": null
            }
        }
    },
    "ADA": {
        "info": {
            "currency": "ADA",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "6",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "Cardano",
            "blockchains": {
                "cardano": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "1",
                    "withdrawal": "enabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "15"
                }
            }
        },
        "id": "ADA",
        "numericId": null,
        "code": "ADA",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 1,
        "fees": {},
        "networks": {
            "ADA": {
                "id": "cardano",
                "network": "ADA",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 1,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 1,
                        "max": null
                    },
                    "withdraw": {
                        "min": 5,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "1",
                    "withdrawal": "enabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "15"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 5,
                "max": null
            },
            "deposit": {
                "min": 1,
                "max": null
            }
        }
    },
    "XRP": {
        "info": {
            "currency": "XRP",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "6",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "XRP",
            "blockchains": {
                "ripple": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.3",
                    "withdrawalFee": "0.25",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            }
        },
        "id": "XRP",
        "numericId": null,
        "code": "XRP",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 0.25,
        "fees": {},
        "networks": {
            "XRP": {
                "id": "ripple",
                "network": "XRP",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 0.25,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 0.001,
                        "max": null
                    },
                    "withdraw": {
                        "min": 0.3,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.3",
                    "withdrawalFee": "0.25",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 0.3,
                "max": null
            },
            "deposit": {
                "min": 0.001,
                "max": null
            }
        }
    },
    "USDC": {
        "info": {
            "currency": "USDC",
            "fiat": false,
            "precision": "8",
            "walletPrecision": "6",
            "walletDeposit": true,
            "walletWithdrawal": true,
            "name": "USDC",
            "blockchains": {
                "solana": {
                    "type": "SPL",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                },
                "tron": {
                    "type": "TRC20",
                    "deposit": "disabled",
                    "minDeposit": "5",
                    "withdrawal": "disabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "5",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "21"
                },
                "stellar": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "2",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                },
                "ethereum": {
                    "type": "ERC20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "20",
                    "withdrawalFee": "10",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                },
                "polygon": {
                    "type": "ERC20",
                    "deposit": "enabled",
                    "minDeposit": "2",
                    "withdrawal": "enabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "128"
                },
                "noble": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.000001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.2",
                    "withdrawalFee": "0.05",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            }
        },
        "id": "USDC",
        "numericId": null,
        "code": "USDC",
        "precision": 1e-8,
        "type": "crypto",
        "name": null,
        "active": true,
        "deposit": true,
        "withdraw": true,
        "fee": 0.05,
        "fees": {},
        "networks": {
            "SOLANA": {
                "id": "solana",
                "network": "SOLANA",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 1,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 10,
                        "max": null
                    }
                },
                "info": {
                    "type": "SPL",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "10",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            },
            "TRC20": {
                "id": "tron",
                "network": "TRC20",
                "margin": null,
                "deposit": false,
                "withdraw": false,
                "active": false,
                "fee": 5,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 5,
                        "max": null
                    }
                },
                "info": {
                    "type": "TRC20",
                    "deposit": "disabled",
                    "minDeposit": "5",
                    "withdrawal": "disabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "5",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "21"
                }
            },
            "XLM": {
                "id": "stellar",
                "network": "XLM",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 2,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 5,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "2",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            },
            "ethereum": {
                "id": "ethereum",
                "network": "ethereum",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 10,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 5,
                        "max": null
                    },
                    "withdraw": {
                        "min": 20,
                        "max": null
                    }
                },
                "info": {
                    "type": "ERC20",
                    "deposit": "enabled",
                    "minDeposit": "5",
                    "withdrawal": "enabled",
                    "minWithdrawal": "20",
                    "withdrawalFee": "10",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "25"
                }
            },
            "MATIC": {
                "id": "polygon",
                "network": "MATIC",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 1,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 2,
                        "max": null
                    },
                    "withdraw": {
                        "min": 5,
                        "max": null
                    }
                },
                "info": {
                    "type": "ERC20",
                    "deposit": "enabled",
                    "minDeposit": "2",
                    "withdrawal": "enabled",
                    "minWithdrawal": "5",
                    "withdrawalFee": "1",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "128"
                }
            },
            "noble": {
                "id": "noble",
                "network": "noble",
                "margin": null,
                "deposit": true,
                "withdraw": true,
                "active": true,
                "fee": 0.05,
                "precision": 1e-8,
                "limits": {
                    "deposit": {
                        "min": 0.000001,
                        "max": null
                    },
                    "withdraw": {
                        "min": 0.2,
                        "max": null
                    }
                },
                "info": {
                    "type": "coin",
                    "deposit": "enabled",
                    "minDeposit": "0.000001",
                    "withdrawal": "enabled",
                    "minWithdrawal": "0.2",
                    "withdrawalFee": "0.05",
                    "withdrawalFeePercent": "0",
                    "depositConfirmations": "1"
                }
            }
        },
        "limits": {
            "amount": {
                "min": null,
                "max": null
            },
            "withdraw": {
                "min": 0.2,
                "max": null
            },
            "deposit": {
                "min": 0.000001,
                "max": null
            }
        }
    }
}
```

## High-Level Overview

This is a JSON file located at `ts/src/test/static/currencies/cex.json`.



## Detailed Walkthrough

### Code Structure

- Total lines: 986
- Code lines: 986
- Comment lines: 0
- Blank lines: 0

### Main Components



## Usage Examples

No explicit usage examples found in the file. Refer to related test files or documentation.



## Performance & Security Notes

No specific performance or security issues detected.



## Related Files

No explicit file references found.



## Testing & Execution

This appears to be a test file.

**To run this test:**
