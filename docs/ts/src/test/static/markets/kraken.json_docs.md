# Documentation: ts/src/test/static/markets/kraken.json

## File Metadata

- **Path**: `ts/src/test/static/markets/kraken.json`
- **Size**: 65,919 bytes
- **Lines**: 2,803
- **Type**: JSON
- **Extension**: .json


## Original Source Code

```json
{
    "BTC/USDT": {
        "id": "XBTUSDT",
        "lowercaseId": null,
        "symbol": "BTC/USDT",
        "base": "BTC",
        "quote": "USDT",
        "settle": null,
        "baseId": "XXBT",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.1
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 3
            },
            "amount": {
                "min": 0.00005,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "XBTUSDT",
            "wsname": "XBT/USDT",
            "aclass_base": "currency",
            "base": "XXBT",
            "aclass_quote": "currency",
            "quote": "USDT",
            "lot": "unit",
            "cost_decimals": "5",
            "pair_decimals": "1",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3
            ],
            "leverage_sell": [
                2,
                3
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "0.00005",
            "costmin": "0.5",
            "tick_size": "0.1",
            "status": "online",
            "long_position_limit": "43",
            "short_position_limit": "40"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "XBT/USDT",
        "altname": "XBTUSDT"
    },
    "BTC/USD": {
        "id": "XXBTZUSD",
        "lowercaseId": null,
        "symbol": "BTC/USD",
        "base": "BTC",
        "quote": "USD",
        "settle": null,
        "baseId": "XXBT",
        "quoteId": "ZUSD",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.1
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 5
            },
            "amount": {
                "min": 0.00005,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "XBTUSD",
            "wsname": "XBT/USD",
            "aclass_base": "currency",
            "base": "XXBT",
            "aclass_quote": "currency",
            "quote": "ZUSD",
            "lot": "unit",
            "cost_decimals": "5",
            "pair_decimals": "1",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3,
                4,
                5
            ],
            "leverage_sell": [
                2,
                3,
                4,
                5
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "0.00005",
            "costmin": "0.5",
            "tick_size": "0.1",
            "status": "online",
            "long_position_limit": "300",
            "short_position_limit": "240"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "XBT/USD",
        "altname": "XBTUSD"
    },
    "ETH/USDT": {
        "id": "ETHUSDT",
        "lowercaseId": null,
        "symbol": "ETH/USDT",
        "base": "ETH",
        "quote": "USDT",
        "settle": null,
        "baseId": "XETH",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.01
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 3
            },
            "amount": {
                "min": 0.002,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "ETHUSDT",
            "wsname": "ETH/USDT",
            "aclass_base": "currency",
            "base": "XETH",
            "aclass_quote": "currency",
            "quote": "USDT",
            "lot": "unit",
            "cost_decimals": "5",
            "pair_decimals": "2",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3
            ],
            "leverage_sell": [
                2,
                3
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "0.002",
            "costmin": "0.5",
            "tick_size": "0.01",
            "status": "online",
            "long_position_limit": "1500",
            "short_position_limit": "1500"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "ETH/USDT",
        "altname": "ETHUSDT"
    },
    "ADA/USDT": {
        "id": "ADAUSDT",
        "lowercaseId": null,
        "symbol": "ADA/USDT",
        "base": "ADA",
        "quote": "USDT",
        "settle": null,
        "baseId": "ADA",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.000001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 4
            },
            "amount": {
                "min": 4.4,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "ADAUSDT",
            "wsname": "ADA/USDT",
            "aclass_base": "currency",
            "base": "ADA",
            "aclass_quote": "currency",
            "quote": "USDT",
            "lot": "unit",
            "cost_decimals": "8",
            "pair_decimals": "6",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3,
                4
            ],
            "leverage_sell": [
                2,
                3,
                4
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "4.4",
            "costmin": "0.5",
            "tick_size": "0.000001",
            "status": "online",
            "long_position_limit": "400000",
            "short_position_limit": "400000"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "ADA/USDT",
        "altname": "ADAUSDT"
    },
    "LTC/USDT": {
        "id": "LTCUSDT",
        "lowercaseId": null,
        "symbol": "LTC/USDT",
        "base": "LTC",
        "quote": "USDT",
        "settle": null,
        "baseId": "XLTC",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.00001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 3
            },
            "amount": {
                "min": 0.05,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "LTCUSDT",
            "wsname": "LTC/USDT",
            "aclass_base": "currency",
            "base": "XLTC",
            "aclass_quote": "currency",
            "quote": "USDT",
            "lot": "unit",
            "cost_decimals": "6",
            "pair_decimals": "5",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3
            ],
            "leverage_sell": [
                2,
                3
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "0.05",
            "costmin": "0.5",
            "tick_size": "0.00001",
            "status": "online",
            "long_position_limit": "1200",
            "short_position_limit": "1200"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "LTC/USDT",
        "altname": "LTCUSDT"
    },
    "XRP/USDT": {
        "id": "XRPUSDT",
        "lowercaseId": null,
        "symbol": "XRP/USDT",
        "base": "XRP",
        "quote": "USDT",
        "settle": null,
        "baseId": "XXRP",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.00001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 3
            },
            "amount": {
                "min": 2,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "XRPUSDT",
            "wsname": "XRP/USDT",
            "aclass_base": "currency",
            "base": "XXRP",
            "aclass_quote": "currency",
            "quote": "USDT",
            "lot": "unit",
            "cost_decimals": "8",
            "pair_decimals": "5",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3
            ],
            "leverage_sell": [
                2,
                3
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "2",
            "costmin": "0.5",
            "tick_size": "0.00001",
            "status": "online",
            "long_position_limit": "260000",
            "short_position_limit": "220000"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "XRP/USDT",
        "altname": "XRPUSDT"
    },
    "DOGE/USDT": {
        "id": "XDGUSDT",
        "lowercaseId": null,
        "symbol": "DOGE/USDT",
        "base": "DOGE",
        "quote": "USDT",
        "settle": null,
        "baseId": "XXDG",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.00001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 4
            },
            "amount": {
                "min": 13,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "XDGUSDT",
            "wsname": "XDG/USDT",
            "aclass_base": "currency",
            "base": "XXDG",
            "aclass_quote": "currency",
            "quote": "USDT",
            "lot": "unit",
            "cost_decimals": "9",
            "pair_decimals": "5",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3,
                4
            ],
            "leverage_sell": [
                2,
                3,
                4
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "13",
            "costmin": "0.5",
            "tick_size": "0.00001",
            "status": "online",
            "long_position_limit": "1100000",
            "short_position_limit": "1100000"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "XDG/USDT",
        "altname": "XDGUSDT"
    },
    "XRP/USD": {
        "id": "XXRPZUSD",
        "lowercaseId": null,
        "symbol": "XRP/USD",
        "base": "XRP",
        "quote": "USD",
        "settle": null,
        "baseId": "XXRP",
        "quoteId": "ZUSD",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.00001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 5
            },
            "amount": {
                "min": 2,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "XRPUSD",
            "wsname": "XRP/USD",
            "aclass_base": "currency",
            "base": "XXRP",
            "aclass_quote": "currency",
            "quote": "ZUSD",
            "lot": "unit",
            "cost_decimals": "8",
            "pair_decimals": "5",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3,
                4,
                5
            ],
            "leverage_sell": [
                2,
                3,
                4,
                5
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "2",
            "costmin": "0.5",
            "tick_size": "0.00001",
            "status": "online",
            "long_position_limit": "2000000",
            "short_position_limit": "1200000"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "XRP/USD",
        "altname": "XRPUSD"
    },
    "LTC/USD": {
        "id": "XLTCZUSD",
        "lowercaseId": null,
        "symbol": "LTC/USD",
        "base": "LTC",
        "quote": "USD",
        "settle": null,
        "baseId": "XLTC",
        "quoteId": "ZUSD",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.01
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 3
            },
            "amount": {
                "min": 0.05,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "LTCUSD",
            "wsname": "LTC/USD",
            "aclass_base": "currency",
            "base": "XLTC",
            "aclass_quote": "currency",
            "quote": "ZUSD",
            "lot": "unit",
            "cost_decimals": "5",
            "pair_decimals": "2",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3
            ],
            "leverage_sell": [
                2,
                3
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "0.05",
            "costmin": "0.5",
            "tick_size": "0.01",
            "status": "online",
            "long_position_limit": "5300",
            "short_position_limit": "3600"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "LTC/USD",
        "altname": "LTCUSD"
    },
    "ALPHA/USD": {
        "id": "ALPHAUSD",
        "lowercaseId": null,
        "symbol": "ALPHA/USD",
        "base": "ALPHA",
        "quote": "USD",
        "settle": null,
        "baseId": "ALPHA",
        "quoteId": "ZUSD",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": false,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.00001,
            "price": 0.00001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 1
            },
            "amount": {
                "min": 50,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "ALPHAUSD",
            "wsname": "ALPHA/USD",
            "aclass_base": "currency",
            "base": "ALPHA",
            "aclass_quote": "currency",
            "quote": "ZUSD",
            "lot": "unit",
            "cost_decimals": "9",
            "pair_decimals": "5",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [],
            "leverage_sell": [],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "50",
            "costmin": "0.5",
            "tick_size": "0.00001",
            "status": "online"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "ALPHA/USD",
        "altname": "ALPHAUSD"
    },
    "ADA/USDC": {
        "id": "ADAUSDC",
        "lowercaseId": null,
        "symbol": "ADA/USDC",
        "base": "ADA",
        "quote": "USDC",
        "settle": null,
        "baseId": "ADA",
        "quoteId": "USDC",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": true,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.004,
        "maker": 0.0025,
        "contractSize": null,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1e-8,
            "price": 0.000001
        },
        "limits": {
            "leverage": {
                "min": 1,
                "max": 10
            },
            "amount": {
                "min": 4.4,
                "max": null
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 0.5,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "altname": "ADAUSDC",
            "wsname": "ADA/USDC",
            "aclass_base": "currency",
            "base": "ADA",
            "aclass_quote": "currency",
            "quote": "USDC",
            "lot": "unit",
            "cost_decimals": "8",
            "pair_decimals": "6",
            "lot_decimals": "8",
            "lot_multiplier": "1",
            "leverage_buy": [
                2,
                3,
                4,
                5,
                6,
                7,
                8,
                9,
                10
            ],
            "leverage_sell": [
                2,
                3,
                4,
                5,
                6,
                7,
                8,
                9,
                10
            ],
            "fees": [
                [
                    0,
                    0.4
                ],
                [
                    10000,
                    0.35
                ],
                [
                    50000,
                    0.24
                ],
                [
                    100000,
                    0.22
                ],
                [
                    250000,
                    0.2
                ],
                [
                    500000,
                    0.18
                ],
                [
                    1000000,
                    0.16
                ],
                [
                    2500000,
                    0.14
                ],
                [
                    5000000,
                    0.12
                ],
                [
                    10000000,
                    0.1
                ]
            ],
            "fees_maker": [
                [
                    0,
                    0.25
                ],
                [
                    10000,
                    0.2
                ],
                [
                    50000,
                    0.14
                ],
                [
                    100000,
                    0.12
                ],
                [
                    250000,
                    0.1
                ],
                [
                    500000,
                    0.08
                ],
                [
                    1000000,
                    0.06
                ],
                [
                    2500000,
                    0.04
                ],
                [
                    5000000,
                    0.02
                ],
                [
                    10000000,
                    0
                ]
            ],
            "fee_volume_currency": "ZUSD",
            "margin_call": "80",
            "margin_stop": "40",
            "ordermin": "4.4",
            "costmin": "0.5",
            "tick_size": "0.000001",
            "status": "online",
            "long_position_limit": "1100000",
            "short_position_limit": "1300000"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0026
                ],
                [
                    50000,
                    0.0024
                ],
                [
                    100000,
                    0.0022
                ],
                [
                    250000,
                    0.002
                ],
                [
                    500000,
                    0.0018
                ],
                [
                    1000000,
                    0.0016
                ],
                [
                    2500000,
                    0.0014
                ],
                [
                    5000000,
                    0.0012
                ],
                [
                    10000000,
                    0.0001
                ]
            ],
            "maker": [
                [
                    0,
                    0.0016
                ],
                [
                    50000,
                    0.0014
                ],
                [
                    100000,
                    0.0012
                ],
                [
                    250000,
                    0.001
                ],
                [
                    500000,
                    0.0008
                ],
                [
                    1000000,
                    0.0006
                ],
                [
                    2500000,
                    0.0004
                ],
                [
                    5000000,
                    0.0002
                ],
                [
                    10000000,
                    0
                ]
            ]
        },
        "wsId": "ADA/USDC",
        "altname": "ADAUSDC"
    }
}
```

## High-Level Overview

This is a JSON file located at `ts/src/test/static/markets/kraken.json`.



## Detailed Walkthrough

### Code Structure

- Total lines: 2803
- Code lines: 2803
- Comment lines: 0
- Blank lines: 0

### Main Components



## Usage Examples

No explicit usage examples found in the file. Refer to related test files or documentation.



## Performance & Security Notes

No specific performance or security issues detected.



## Related Files

No explicit file references found.



## Testing & Execution

This appears to be a test file.

**To run this test:**
