# Documentation: ts/src/test/static/markets/delta.json

## File Metadata

- **Path**: `ts/src/test/static/markets/delta.json`
- **Size**: 68,238 bytes
- **Lines**: 1,804
- **Type**: JSON
- **Extension**: .json


## Original Source Code

```json
{
    "BTC/USDT": {
        "id": "BTC_USDT",
        "lowercaseId": null,
        "symbol": "BTC/USDT",
        "base": "BTC",
        "quote": "USDT",
        "settle": null,
        "baseId": "BTC",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": null,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.00075,
        "maker": 0.00075,
        "contractSize": 1,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.000001,
            "price": 0.1
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 1,
                "max": 10000000
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1624442834000,
        "info": {
            "quoting_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": "5",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": "2",
                "name": "Tether",
                "networks": [
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "6.000000000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "0",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "1.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "1",
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "symbol": "BTC_USDT",
            "maintenance_margin_scaling_factor": "0",
            "initial_margin": "1",
            "maintenance_margin": "0",
            "state": "live",
            "id": "8320",
            "launch_time": "2021-06-23T10:07:14Z",
            "barrier_price": null,
            "insurance_fund_margin_contribution": "1",
            "contract_type": "spot",
            "settlement_time": null,
            "notional_type": "vanilla",
            "product_specs": {
                "kyc_required": false,
                "max_order_size": "10",
                "min_order_size": "5.0e-6",
                "only_reduce_only_orders_allowed": false,
                "quoting_precision": "5",
                "underlying_precision": "6"
            },
            "position_size_limit": "10000000",
            "underlying_asset": {
                "base_withdrawal_fee": "0.000500000000000000",
                "id": "2",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "10.000000000000000000",
                "kyc_withdrawal_limit": "0.500000000000000000",
                "min_withdrawal_amount": "0.001000000000000000",
                "minimum_precision": "4",
                "name": "Bitcoin",
                "networks": [
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.001",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000000000000000000",
                        "min_withdrawal_amount": "0.002000000000000000",
                        "minimum_deposit_confirmations": "2",
                        "network": "BTC",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.00002",
                        "deposit_status": "disabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "0.000100000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "2",
                "symbol": "BTC",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "is_quanto": false,
            "strike_price": null,
            "price_band": "100",
            "max_leverage_notional": "10000000",
            "impact_size": "2",
            "funding_method": "mark_price",
            "contract_value": "1",
            "auction_start_time": "2025-05-06T06:38:14Z",
            "short_description": "BTC-USDT spot market",
            "disruption_reason": null,
            "annualized_funding": "0",
            "contract_unit_currency": "BTC",
            "auction_finish_time": null,
            "settling_asset": null,
            "trading_status": "operational",
            "ui_config": {
                "default_trading_view_candle": "1d",
                "leverage_slider_values": [],
                "price_clubbing_values": [
                    0.1,
                    0.5,
                    1,
                    5,
                    25,
                    100,
                    500
                ],
                "show_bracket_orders": false,
                "sort_priority": "2",
                "tags": []
            },
            "settlement_price": null,
            "default_leverage": "1.000000000000000000",
            "taker_commission_rate": "0.00075",
            "basis_factor_max_limit": "10000",
            "maker_commission_rate": "0.00075",
            "initial_margin_scaling_factor": "0",
            "liquidation_penalty_factor": "1",
            "tick_size": "0.1",
            "spot_index": {
                "config": {
                    "impact_size": {
                        "max_impact_size": "150000",
                        "min_impact_size": "5000",
                        "step_value": "5000"
                    },
                    "price_alert_enabled": true,
                    "quoting_asset": "USDT",
                    "service_id": "1",
                    "underlying_asset": "BTC"
                },
                "constituent_exchanges": [
                    {
                        "exchange": "binance",
                        "health_interval": "60",
                        "health_priority": "1",
                        "weight": "1"
                    },
                    {
                        "exchange": "okex",
                        "health_interval": "60",
                        "health_priority": "2",
                        "weight": "1"
                    },
                    {
                        "exchange": "bybit",
                        "health_interval": "60",
                        "health_priority": "3",
                        "weight": "1"
                    }
                ],
                "constituent_indices": null,
                "description": "Bitcoin spot price index quoted in USDT",
                "health_interval": "300",
                "id": "2",
                "impact_size": "1.000000000000000000",
                "index_type": "spot_pair",
                "is_composite": false,
                "price_method": "orderbook",
                "quoting_asset_id": "5",
                "symbol": ".DEXBTUSDT",
                "tick_size": "0.500000000000000000",
                "underlying_asset_id": "2"
            },
            "description": "Bitcoin Tether spot market"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0015
                ],
                [
                    100,
                    0.0013
                ],
                [
                    250,
                    0.0013
                ],
                [
                    1000,
                    0.001
                ],
                [
                    5000,
                    0.0009
                ],
                [
                    10000,
                    0.00075
                ],
                [
                    20000,
                    0.00065
                ]
            ],
            "maker": [
                [
                    0,
                    0.001
                ],
                [
                    100,
                    0.001
                ],
                [
                    250,
                    0.0009
                ],
                [
                    1000,
                    0.00075
                ],
                [
                    5000,
                    0.0006
                ],
                [
                    10000,
                    0.0005
                ],
                [
                    20000,
                    0.0005
                ]
            ]
        },
        "numericId": 8320
    },
    "BTC/USDT:USDT": {
        "id": "BTCUSDT",
        "lowercaseId": null,
        "symbol": "BTC/USDT:USDT",
        "base": "BTC",
        "quote": "USDT",
        "settle": "USDT",
        "baseId": "BTC",
        "quoteId": "USDT",
        "settleId": "USDT",
        "type": "swap",
        "spot": false,
        "margin": false,
        "swap": true,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": true,
        "linear": true,
        "inverse": false,
        "subType": "linear",
        "taker": 0.0002,
        "maker": 0.0002,
        "contractSize": 0.001,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1,
            "price": 0.5
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 1,
                "max": 229167
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1586011758000,
        "info": {
            "quoting_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": "5",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": "2",
                "name": "Tether",
                "networks": [
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "6.000000000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "0",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "1.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "1",
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "symbol": "BTCUSDT",
            "maintenance_margin_scaling_factor": "0.000001",
            "initial_margin": "1",
            "maintenance_margin": "0.5",
            "state": "live",
            "id": "139",
            "launch_time": "2020-04-04T14:49:18Z",
            "barrier_price": null,
            "insurance_fund_margin_contribution": "5",
            "contract_type": "perpetual_futures",
            "settlement_time": null,
            "notional_type": "vanilla",
            "product_specs": {
                "funding_clamp_value": "0.05",
                "only_reduce_only_orders_allowed": false,
                "tags": [
                    "layer_1"
                ]
            },
            "position_size_limit": "229167",
            "underlying_asset": {
                "base_withdrawal_fee": "0.000500000000000000",
                "id": "2",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "10.000000000000000000",
                "kyc_withdrawal_limit": "0.500000000000000000",
                "min_withdrawal_amount": "0.001000000000000000",
                "minimum_precision": "4",
                "name": "Bitcoin",
                "networks": [
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.001",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000000000000000000",
                        "min_withdrawal_amount": "0.002000000000000000",
                        "minimum_deposit_confirmations": "2",
                        "network": "BTC",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.00002",
                        "deposit_status": "disabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "0.000100000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "2",
                "symbol": "BTC",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "is_quanto": false,
            "strike_price": null,
            "price_band": "5",
            "max_leverage_notional": "200000",
            "impact_size": "10000",
            "funding_method": "mark_price",
            "contract_value": "0.001",
            "auction_start_time": "2025-05-06T06:38:14Z",
            "short_description": "Bitcoin Perpetual",
            "disruption_reason": null,
            "annualized_funding": "10.95",
            "contract_unit_currency": "BTC",
            "auction_finish_time": null,
            "settling_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": "5",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": "2",
                "name": "Tether",
                "networks": [
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "6.000000000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "0",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "1.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "1",
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "trading_status": "operational",
            "ui_config": {
                "default_trading_view_candle": "15",
                "leverage_slider_values": [
                    1,
                    2,
                    5,
                    10,
                    25,
                    50,
                    100
                ],
                "price_clubbing_values": [
                    0.5,
                    1,
                    10,
                    50
                ],
                "show_bracket_orders": false,
                "sort_priority": "1",
                "tags": []
            },
            "settlement_price": null,
            "default_leverage": "100.000000000000000000",
            "taker_commission_rate": "0.0002",
            "basis_factor_max_limit": "10.95",
            "maker_commission_rate": "0.0002",
            "initial_margin_scaling_factor": "0.000002",
            "liquidation_penalty_factor": "0.2",
            "tick_size": "0.5",
            "spot_index": {
                "config": {
                    "impact_size": {
                        "max_impact_size": "150000",
                        "min_impact_size": "5000",
                        "step_value": "5000"
                    },
                    "price_alert_enabled": true,
                    "quoting_asset": "USDT",
                    "service_id": "1",
                    "underlying_asset": "BTC"
                },
                "constituent_exchanges": [
                    {
                        "exchange": "binance",
                        "health_interval": "60",
                        "health_priority": "1",
                        "weight": "1"
                    },
                    {
                        "exchange": "okex",
                        "health_interval": "60",
                        "health_priority": "2",
                        "weight": "1"
                    },
                    {
                        "exchange": "bybit",
                        "health_interval": "60",
                        "health_priority": "3",
                        "weight": "1"
                    }
                ],
                "constituent_indices": null,
                "description": "Bitcoin spot price index quoted in USDT",
                "health_interval": "300",
                "id": "2",
                "impact_size": "1.000000000000000000",
                "index_type": "spot_pair",
                "is_composite": false,
                "price_method": "orderbook",
                "quoting_asset_id": "5",
                "symbol": ".DEXBTUSDT",
                "tick_size": "0.500000000000000000",
                "underlying_asset_id": "2"
            },
            "description": "Bitcoin Perpetual futures, quoted,settled & margined in Tether"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0015
                ],
                [
                    100,
                    0.0013
                ],
                [
                    250,
                    0.0013
                ],
                [
                    1000,
                    0.001
                ],
                [
                    5000,
                    0.0009
                ],
                [
                    10000,
                    0.00075
                ],
                [
                    20000,
                    0.00065
                ]
            ],
            "maker": [
                [
                    0,
                    0.001
                ],
                [
                    100,
                    0.001
                ],
                [
                    250,
                    0.0009
                ],
                [
                    1000,
                    0.00075
                ],
                [
                    5000,
                    0.0006
                ],
                [
                    10000,
                    0.0005
                ],
                [
                    20000,
                    0.0005
                ]
            ]
        },
        "numericId": 139
    },
    "ETH/USDT": {
        "id": "ETH_USDT",
        "lowercaseId": null,
        "symbol": "ETH/USDT",
        "base": "ETH",
        "quote": "USDT",
        "settle": null,
        "baseId": "ETH",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": null,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0.00075,
        "maker": 0.00075,
        "contractSize": 1,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.00001,
            "price": 0.01
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 1,
                "max": 10000000
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1624888978000,
        "info": {
            "quoting_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": "5",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": "2",
                "name": "Tether",
                "networks": [
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "6.000000000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "0",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "1.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "1",
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "symbol": "ETH_USDT",
            "maintenance_margin_scaling_factor": "0",
            "initial_margin": "1",
            "maintenance_margin": "0",
            "state": "live",
            "id": "8411",
            "launch_time": "2021-06-28T14:02:58Z",
            "barrier_price": null,
            "insurance_fund_margin_contribution": "1",
            "contract_type": "spot",
            "settlement_time": null,
            "notional_type": "vanilla",
            "product_specs": {
                "kyc_required": false,
                "max_order_size": "10",
                "min_order_size": "0.001",
                "only_reduce_only_orders_allowed": false,
                "quoting_precision": "5",
                "underlying_precision": "5"
            },
            "position_size_limit": "10000000",
            "underlying_asset": {
                "base_withdrawal_fee": "0.005000000000000000",
                "id": "1",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "0.000000000000000000",
                "kyc_withdrawal_limit": "0.000000000000000000",
                "min_withdrawal_amount": "0.010000000000000000",
                "minimum_precision": "4",
                "name": "Ethereum",
                "networks": [
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.0025",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000050000000000000",
                        "min_withdrawal_amount": "0.010000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.0001",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000050000000000000",
                        "min_withdrawal_amount": "0.000300000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "18",
                "sort_priority": "3",
                "symbol": "ETH",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "is_quanto": false,
            "strike_price": null,
            "price_band": "100",
            "max_leverage_notional": "10000000",
            "impact_size": "2",
            "funding_method": "mark_price",
            "contract_value": "1",
            "auction_start_time": "2025-05-06T06:38:14Z",
            "short_description": "ETH-USDT spot market",
            "disruption_reason": null,
            "annualized_funding": "0",
            "contract_unit_currency": "ETH",
            "auction_finish_time": null,
            "settling_asset": null,
            "trading_status": "operational",
            "ui_config": {
                "default_trading_view_candle": "1d",
                "leverage_slider_values": [],
                "price_clubbing_values": [
                    0.01,
                    0.05,
                    0.1,
                    0.5,
                    1,
                    5,
                    25,
                    100
                ],
                "show_bracket_orders": false,
                "sort_priority": "2",
                "tags": []
            },
            "settlement_price": null,
            "default_leverage": "1.000000000000000000",
            "taker_commission_rate": "0.00075",
            "basis_factor_max_limit": "10000",
            "maker_commission_rate": "0.00075",
            "initial_margin_scaling_factor": "0",
            "liquidation_penalty_factor": "1",
            "tick_size": "0.01",
            "spot_index": {
                "config": {
                    "impact_size": {
                        "max_impact_size": "150000",
                        "min_impact_size": "5000",
                        "step_value": "5000"
                    },
                    "price_alert_enabled": true,
                    "quoting_asset": "USDT",
                    "service_id": "1",
                    "underlying_asset": "ETH"
                },
                "constituent_exchanges": [
                    {
                        "exchange": "binance",
                        "health_interval": "60",
                        "health_priority": "1",
                        "weight": "1"
                    },
                    {
                        "exchange": "okex",
                        "health_interval": "60",
                        "health_priority": "2",
                        "weight": "1"
                    },
                    {
                        "exchange": "bybit",
                        "health_interval": "60",
                        "health_priority": "3",
                        "weight": "1"
                    }
                ],
                "constituent_indices": null,
                "description": "ETH/USDT price on Binance, Okex and Bybit",
                "health_interval": "300",
                "id": "49",
                "impact_size": "10.000000000000000000",
                "index_type": "spot_pair",
                "is_composite": false,
                "price_method": "orderbook",
                "quoting_asset_id": "5",
                "symbol": ".DEETHUSDT",
                "tick_size": "0.050000000000000000",
                "underlying_asset_id": "1"
            },
            "description": "Ethereum Tether spot market"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0015
                ],
                [
                    100,
                    0.0013
                ],
                [
                    250,
                    0.0013
                ],
                [
                    1000,
                    0.001
                ],
                [
                    5000,
                    0.0009
                ],
                [
                    10000,
                    0.00075
                ],
                [
                    20000,
                    0.00065
                ]
            ],
            "maker": [
                [
                    0,
                    0.001
                ],
                [
                    100,
                    0.001
                ],
                [
                    250,
                    0.0009
                ],
                [
                    1000,
                    0.00075
                ],
                [
                    5000,
                    0.0006
                ],
                [
                    10000,
                    0.0005
                ],
                [
                    20000,
                    0.0005
                ]
            ]
        },
        "numericId": 8411
    },
    "ETH/USDT:USDT": {
        "id": "ETHUSDT",
        "lowercaseId": null,
        "symbol": "ETH/USDT:USDT",
        "base": "ETH",
        "quote": "USDT",
        "settle": "USDT",
        "baseId": "ETH",
        "quoteId": "USDT",
        "settleId": "USDT",
        "type": "swap",
        "spot": false,
        "margin": false,
        "swap": true,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": true,
        "linear": true,
        "inverse": false,
        "subType": "linear",
        "taker": 0.0002,
        "maker": 0.0002,
        "contractSize": 0.01,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1,
            "price": 0.05
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 1,
                "max": 162683
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1587564812000,
        "info": {
            "quoting_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": "5",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": "2",
                "name": "Tether",
                "networks": [
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "6.000000000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "0",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "1.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "1",
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "symbol": "ETHUSDT",
            "maintenance_margin_scaling_factor": "0.000002",
            "initial_margin": "1",
            "maintenance_margin": "0.5",
            "state": "live",
            "id": "176",
            "launch_time": "2020-04-22T14:13:32Z",
            "barrier_price": null,
            "insurance_fund_margin_contribution": "5",
            "contract_type": "perpetual_futures",
            "settlement_time": null,
            "notional_type": "vanilla",
            "product_specs": {
                "funding_clamp_value": "0.05",
                "only_reduce_only_orders_allowed": false,
                "tags": [
                    "smart_contracts",
                    "layer_1"
                ]
            },
            "position_size_limit": "162683",
            "underlying_asset": {
                "base_withdrawal_fee": "0.005000000000000000",
                "id": "1",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "0.000000000000000000",
                "kyc_withdrawal_limit": "0.000000000000000000",
                "min_withdrawal_amount": "0.010000000000000000",
                "minimum_precision": "4",
                "name": "Ethereum",
                "networks": [
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.0025",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000050000000000000",
                        "min_withdrawal_amount": "0.010000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": null,
                        "base_withdrawal_fee": "0.0001",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000050000000000000",
                        "min_withdrawal_amount": "0.000300000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "18",
                "sort_priority": "3",
                "symbol": "ETH",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "is_quanto": false,
            "strike_price": null,
            "price_band": "5",
            "max_leverage_notional": "100000",
            "impact_size": "3000",
            "funding_method": "mark_price",
            "contract_value": "0.01",
            "auction_start_time": "2025-05-06T06:38:14Z",
            "short_description": "Ether Perpetual",
            "disruption_reason": null,
            "annualized_funding": "10.95",
            "contract_unit_currency": "ETH",
            "auction_finish_time": null,
            "settling_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": "5",
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": "2",
                "name": "Tether",
                "networks": [
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": "12",
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "6.000000000000000000",
                        "minimum_deposit_confirmations": "15",
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "allowed_deposit_groups": "G1",
                        "base_withdrawal_fee": "0",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "1.000000000000000000",
                        "minimum_deposit_confirmations": "1",
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": "8",
                "sort_priority": "1",
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "trading_status": "operational",
            "ui_config": {
                "default_trading_view_candle": "15",
                "leverage_slider_values": [
                    1,
                    2,
                    5,
                    10,
                    25,
                    50,
                    100
                ],
                "price_clubbing_values": [
                    0.05,
                    0.1,
                    1,
                    10
                ],
                "show_bracket_orders": false,
                "sort_priority": "3",
                "tags": []
            },
            "settlement_price": null,
            "default_leverage": "100.000000000000000000",
            "taker_commission_rate": "0.0002",
            "basis_factor_max_limit": "10.95",
            "maker_commission_rate": "0.0002",
            "initial_margin_scaling_factor": "0.000004",
            "liquidation_penalty_factor": "0.2",
            "tick_size": "0.05",
            "spot_index": {
                "config": {
                    "impact_size": {
                        "max_impact_size": "150000",
                        "min_impact_size": "5000",
                        "step_value": "5000"
                    },
                    "price_alert_enabled": true,
                    "quoting_asset": "USDT",
                    "service_id": "1",
                    "underlying_asset": "ETH"
                },
                "constituent_exchanges": [
                    {
                        "exchange": "binance",
                        "health_interval": "60",
                        "health_priority": "1",
                        "weight": "1"
                    },
                    {
                        "exchange": "okex",
                        "health_interval": "60",
                        "health_priority": "2",
                        "weight": "1"
                    },
                    {
                        "exchange": "bybit",
                        "health_interval": "60",
                        "health_priority": "3",
                        "weight": "1"
                    }
                ],
                "constituent_indices": null,
                "description": "ETH/USDT price on Binance, Okex and Bybit",
                "health_interval": "300",
                "id": "49",
                "impact_size": "10.000000000000000000",
                "index_type": "spot_pair",
                "is_composite": false,
                "price_method": "orderbook",
                "quoting_asset_id": "5",
                "symbol": ".DEETHUSDT",
                "tick_size": "0.050000000000000000",
                "underlying_asset_id": "1"
            },
            "description": "Ethereum Perpetual futures, quoted,settled & margined in Tether"
        },
        "tierBased": true,
        "percentage": true,
        "tiers": {
            "taker": [
                [
                    0,
                    0.0015
                ],
                [
                    100,
                    0.0013
                ],
                [
                    250,
                    0.0013
                ],
                [
                    1000,
                    0.001
                ],
                [
                    5000,
                    0.0009
                ],
                [
                    10000,
                    0.00075
                ],
                [
                    20000,
                    0.00065
                ]
            ],
            "maker": [
                [
                    0,
                    0.001
                ],
                [
                    100,
                    0.001
                ],
                [
                    250,
                    0.0009
                ],
                [
                    1000,
                    0.00075
                ],
                [
                    5000,
                    0.0006
                ],
                [
                    10000,
                    0.0005
                ],
                [
                    20000,
                    0.0005
                ]
            ]
        },
        "numericId": 176
    },
    "BTC/USDT:USDT-240329-65000-P": {
        "id": "P-BTC-65000-290324",
        "numericId": 161274,
        "symbol": "BTC/USDT:USDT-240329-65000-P",
        "base": "BTC",
        "quote": "USDT",
        "settle": "USDT",
        "baseId": "BTC",
        "quoteId": "USDT",
        "settleId": "USDT",
        "type": "option",
        "spot": false,
        "margin": false,
        "swap": false,
        "future": false,
        "option": true,
        "active": true,
        "contract": true,
        "linear": false,
        "inverse": true,
        "taker": 0.000375,
        "maker": 0.000375,
        "contractSize": 0.001,
        "precision": {
            "amount": 1,
            "price": 0.1
        },
        "limits": {
            "leverage": {},
            "amount": {
                "min": 1,
                "max": 255633
            },
            "price": {},
            "cost": {}
        },
        "expiry": 1711713600000,
        "expiryDatetime": "2024-03-29T12:00:00Z",
        "strike": 65000,
        "optionType": "put",
        "created": 1707499801000,
        "info": {
            "contract_unit_currency": "BTC",
            "trading_status": "operational",
            "description": "BTC put option expiring on 29-3-2024",
            "max_leverage_notional": "200000",
            "is_quanto": false,
            "position_size_limit": 255633,
            "taker_commission_rate": "0.000375",
            "maker_commission_rate": "0.000375",
            "tick_size": "0.1",
            "auction_finish_time": null,
            "id": 161274,
            "notional_type": "vanilla",
            "auction_start_time": null,
            "spot_index": {
                "config": {
                    "impact_size": {
                        "max_impact_size": 150000,
                        "min_impact_size": 5000,
                        "step_value": 5000
                    },
                    "quoting_asset": "USDT",
                    "service_id": 1,
                    "underlying_asset": "BTC"
                },
                "constituent_exchanges": [
                    {
                        "exchange": "binance",
                        "health_interval": 60,
                        "health_priority": 1,
                        "weight": 1
                    },
                    {
                        "exchange": "okex",
                        "health_interval": 60,
                        "health_priority": 2,
                        "weight": 1
                    },
                    {
                        "exchange": "bybit",
                        "health_interval": 60,
                        "health_priority": 3,
                        "weight": 1
                    }
                ],
                "constituent_indices": null,
                "description": "Bitcoin spot price index quoted in USDT",
                "health_interval": 300,
                "id": 2,
                "impact_size": "1.000000000000000000",
                "index_type": "spot_pair",
                "is_composite": false,
                "price_method": "orderbook",
                "quoting_asset_id": 5,
                "symbol": ".DEXBTUSDT",
                "tick_size": "0.500000000000000000",
                "underlying_asset_id": 2
            },
            "price_band": "2",
            "contract_type": "put_options",
            "insurance_fund_margin_contribution": "1",
            "settlement_time": "2024-03-29T12:00:00Z",
            "initial_margin_scaling_factor": "0.000002",
            "maintenance_margin_scaling_factor": "0.000001",
            "settling_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": 5,
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": 2,
                "name": "Tether",
                "networks": [
                    {
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": 12,
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": 15,
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": 1,
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "0.5",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": 1,
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": 8,
                "sort_priority": 1,
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "impact_size": 200,
            "symbol": "P-BTC-65000-290324",
            "strike_price": "65000",
            "maintenance_margin": "0.5",
            "short_description": "BTC  Put",
            "liquidation_penalty_factor": "0.5",
            "default_leverage": "100.000000000000000000",
            "contract_value": "0.001",
            "annualized_funding": "10.95",
            "state": "live",
            "quoting_asset": {
                "base_withdrawal_fee": "10.000000000000000000",
                "id": 5,
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "100000.000000000000000000",
                "kyc_withdrawal_limit": "10000.000000000000000000",
                "min_withdrawal_amount": "30.000000000000000000",
                "minimum_precision": 2,
                "name": "Tether",
                "networks": [
                    {
                        "base_withdrawal_fee": "10",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": 12,
                        "network": "ERC20",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "1",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": 15,
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "3",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "1.000000000000000000",
                        "min_withdrawal_amount": "30.000000000000000000",
                        "minimum_deposit_confirmations": 1,
                        "network": "TRC20(TRON)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "0.5",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "10.000000000000000000",
                        "minimum_deposit_confirmations": 1,
                        "network": "POLYGON",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": 8,
                "sort_priority": 1,
                "symbol": "USDT",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "barrier_price": null,
            "basis_factor_max_limit": "10.95",
            "settlement_price": null,
            "product_specs": {
                "backup_vol_expiry_time": 31536000,
                "max_deviation_from_external_vol": 0.75,
                "max_lower_deviation_from_external_vol": 0.75,
                "max_upper_deviation_from_external_vol": 0.5,
                "max_volatility": 3,
                "min_volatility": 0.1,
                "premium_commission_rate": 0.125,
                "vol_calculation_method": "orderbook",
                "vol_expiry_time": 31536000
            },
            "initial_margin": "1",
            "ui_config": {
                "default_trading_view_candle": "15",
                "leverage_slider_values": [
                    1,
                    2,
                    5,
                    10,
                    25,
                    50,
                    100
                ],
                "price_clubbing_values": [
                    0.01,
                    0.05,
                    0.1,
                    0.5,
                    1,
                    5,
                    10
                ],
                "show_bracket_orders": false,
                "sort_priority": 30,
                "tags": []
            },
            "launch_time": "2024-02-09T17:30:01Z",
            "disruption_reason": null,
            "underlying_asset": {
                "base_withdrawal_fee": "0.000500000000000000",
                "id": 2,
                "interest_credit": false,
                "interest_slabs": null,
                "kyc_deposit_limit": "10.000000000000000000",
                "kyc_withdrawal_limit": "0.500000000000000000",
                "min_withdrawal_amount": "0.001000000000000000",
                "minimum_precision": 4,
                "name": "Bitcoin",
                "networks": [
                    {
                        "base_withdrawal_fee": "0.001",
                        "deposit_status": "enabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.000000000000000000",
                        "min_withdrawal_amount": "0.002000000000000000",
                        "minimum_deposit_confirmations": 2,
                        "network": "BTC",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    },
                    {
                        "base_withdrawal_fee": "0.00002",
                        "deposit_status": "disabled",
                        "memo_required": false,
                        "min_deposit_amount": "0.500000000000000000",
                        "min_withdrawal_amount": "0.000100000000000000",
                        "minimum_deposit_confirmations": 1,
                        "network": "BEP20(BSC)",
                        "variable_withdrawal_fee": "0",
                        "withdrawal_status": "enabled"
                    }
                ],
                "precision": 8,
                "sort_priority": 2,
                "symbol": "BTC",
                "variable_withdrawal_fee": "0.000000000000000000"
            },
            "funding_method": "mark_price"
        }
    }
}
```

## High-Level Overview

This is a JSON file located at `ts/src/test/static/markets/delta.json`.



## Detailed Walkthrough

### Code Structure

- Total lines: 1804
- Code lines: 1804
- Comment lines: 0
- Blank lines: 0

### Main Components



## Usage Examples

No explicit usage examples found in the file. Refer to related test files or documentation.



## Performance & Security Notes

No specific performance or security issues detected.



## Related Files

No explicit file references found.



## Testing & Execution

This appears to be a test file.

**To run this test:**
