# Documentation: ts/src/test/static/markets/paradex.json

## File Metadata

- **Path**: `ts/src/test/static/markets/paradex.json`
- **Size**: 15,032 bytes
- **Lines**: 503
- **Type**: JSON
- **Extension**: .json


## Original Source Code

```json
{
    "BTC/USD:USDC": {
        "id": "BTC-USD-PERP",
        "symbol": "BTC/USD:USDC",
        "base": "BTC",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "BTC",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "swap",
        "spot": false,
        "swap": true,
        "future": false,
        "option": false,
        "contract": true,
        "linear": true,
        "subType": "linear",
        "taker": 0.0003,
        "maker": -0.00005,
        "contractSize": 1,
        "precision": {
            "amount": 0.001,
            "price": 0.1
        },
        "limits": {
            "leverage": {},
            "amount": {
                "max": 100
            },
            "price": {},
            "cost": {
                "min": 500
            }
        },
        "info": {
            "symbol": "BTC-USD-PERP",
            "base_currency": "BTC",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "0.001",
            "price_tick_size": "0.1",
            "min_notional": "500",
            "open_at": "1696431006398",
            "expiry_at": "0",
            "asset_kind": "PERP",
            "market_kind": "cross",
            "position_limit": "100",
            "price_bands_width": "0.05",
            "max_open_orders": "100",
            "max_funding_rate": "0.02",
            "delta1_cross_margin_params": {
                "imf_base": "0.02",
                "imf_shift": "180000",
                "imf_factor": "0.00007",
                "mmf_factor": "0.5"
            },
            "price_feed_id": "GVXRSBjFk6e6J3NbVPXohDJetcTjaeeuykUpbQF8UoMU",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "100",
            "max_funding_rate_change": "0.0002",
            "max_tob_spread": "0.1",
            "interest_rate": "0.0001",
            "clamp_rate": "0.0005"
        }
    },
    "SOL/USD:USDC": {
        "id": "SOL-USD-PERP",
        "symbol": "SOL/USD:USDC",
        "base": "SOL",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "SOL",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "swap",
        "spot": false,
        "swap": true,
        "future": false,
        "option": false,
        "contract": true,
        "linear": true,
        "subType": "linear",
        "taker": 0.0003,
        "maker": -0.00005,
        "contractSize": 1,
        "precision": {
            "amount": 0.1,
            "price": 0.001
        },
        "limits": {
            "leverage": {},
            "amount": {
                "max": 10000
            },
            "price": {},
            "cost": {
                "min": 200
            }
        },
        "info": {
            "symbol": "SOL-USD-PERP",
            "base_currency": "SOL",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "0.1",
            "price_tick_size": "0.001",
            "min_notional": "200",
            "open_at": "1697783400000",
            "expiry_at": "0",
            "asset_kind": "PERP",
            "market_kind": "cross",
            "position_limit": "20000",
            "price_bands_width": "0.1",
            "max_open_orders": "100",
            "max_funding_rate": "0.05",
            "delta1_cross_margin_params": {
                "imf_base": "0.05",
                "imf_shift": "200000",
                "imf_factor": "0.0002",
                "mmf_factor": "0.5"
            },
            "price_feed_id": "H6ARHf6YXhGYeQfUzQNGk6rDNnLBQKrenN712K4AQJEG",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "10000",
            "max_funding_rate_change": "0.0002",
            "max_tob_spread": "0.1",
            "interest_rate": "0.0001",
            "clamp_rate": "0.0005"
        }
    },
    "ETH/USD:USDC": {
        "id": "ETH-USD-PERP",
        "symbol": "ETH/USD:USDC",
        "base": "ETH",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "ETH",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "swap",
        "spot": false,
        "swap": true,
        "future": false,
        "option": false,
        "contract": true,
        "linear": true,
        "subType": "linear",
        "taker": 0.0003,
        "maker": -0.00005,
        "contractSize": 1,
        "precision": {
            "amount": 0.001,
            "price": 0.01
        },
        "limits": {
            "leverage": {},
            "amount": {
                "max": 1000
            },
            "price": {},
            "cost": {
                "min": 500
            }
        },
        "info": {
            "symbol": "ETH-USD-PERP",
            "base_currency": "ETH",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "0.001",
            "price_tick_size": "0.01",
            "min_notional": "500",
            "open_at": "1692576000000",
            "expiry_at": "0",
            "asset_kind": "PERP",
            "market_kind": "cross",
            "position_limit": "2000",
            "price_bands_width": "0.05",
            "max_open_orders": "100",
            "max_funding_rate": "0.02",
            "delta1_cross_margin_params": {
                "imf_base": "0.02",
                "imf_shift": "180000",
                "imf_factor": "0.00007",
                "mmf_factor": "0.5"
            },
            "price_feed_id": "JBu1AL4obBcCMqKBBxhpWCNUt136ijcuMZLFvTP7iWdB",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "1000",
            "max_funding_rate_change": "0.0002",
            "max_tob_spread": "0.1",
            "interest_rate": "0.0001",
            "clamp_rate": "0.0005"
        }
    },
    "TON/USD:USDC": {
        "id": "TON-USD-PERP",
        "symbol": "TON/USD:USDC",
        "base": "TON",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "TON",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "swap",
        "spot": false,
        "swap": true,
        "future": false,
        "option": false,
        "contract": true,
        "linear": true,
        "subType": "linear",
        "taker": 0.0003,
        "maker": -0.00005,
        "contractSize": 1,
        "precision": {
            "amount": 0.1,
            "price": 0.0001
        },
        "limits": {
            "leverage": {},
            "amount": {
                "max": 90000
            },
            "price": {},
            "cost": {
                "min": 200
            }
        },
        "info": {
            "symbol": "TON-USD-PERP",
            "base_currency": "TON",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "0.1",
            "price_tick_size": "0.0001",
            "min_notional": "200",
            "open_at": "1714374000000",
            "expiry_at": "0",
            "asset_kind": "PERP",
            "market_kind": "cross",
            "position_limit": "350000",
            "price_bands_width": "0.1",
            "max_open_orders": "50",
            "max_funding_rate": "0.05",
            "delta1_cross_margin_params": {
                "imf_base": "0.1",
                "imf_shift": "180000",
                "imf_factor": "0.00035",
                "mmf_factor": "0.5"
            },
            "price_feed_id": "AFJXXYuniABNnoEE7DLtkxwqLDkcda4xG5k2F4FB86hj",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "90000",
            "max_funding_rate_change": "0.0005",
            "max_tob_spread": "0.2",
            "interest_rate": "0.0001",
            "clamp_rate": "0.0005"
        }
    },
    "XRP/USD:USDC": {
        "id": "XRP-USD-PERP",
        "symbol": "XRP/USD:USDC",
        "base": "XRP",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "XRP",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "swap",
        "spot": false,
        "swap": true,
        "future": false,
        "option": false,
        "contract": true,
        "linear": true,
        "subType": "linear",
        "taker": 0.0003,
        "maker": -0.00005,
        "contractSize": 1,
        "precision": {
            "amount": 1,
            "price": 0.00001
        },
        "limits": {
            "leverage": {},
            "amount": {
                "max": 1000000
            },
            "price": {},
            "cost": {
                "min": 200
            }
        },
        "info": {
            "symbol": "XRP-USD-PERP",
            "base_currency": "XRP",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "1",
            "price_tick_size": "0.00001",
            "min_notional": "200",
            "open_at": "1715925600000",
            "expiry_at": "0",
            "asset_kind": "PERP",
            "market_kind": "cross",
            "position_limit": "1000000",
            "price_bands_width": "0.1",
            "max_open_orders": "50",
            "max_funding_rate": "0.05",
            "delta1_cross_margin_params": {
                "imf_base": "0.1",
                "imf_shift": "180000",
                "imf_factor": "0.00035",
                "mmf_factor": "0.5"
            },
            "price_feed_id": "Guffb8DAAxNH6kdoawYjPXTbwUhjmveh8R4LM6uEqRV1",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "1000000",
            "max_funding_rate_change": "0.0005",
            "max_tob_spread": "0.2",
            "interest_rate": "0.0001",
            "clamp_rate": "0.0005"
        }
    },
    "BTC/USD:USDC-117000-P": {
        "id": "BTC-USD-117000-P",
        "symbol": "BTC/USD:USDC-117000-P",
        "base": "BTC",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "BTC",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "option",
        "spot": false,
        "swap": false,
        "future": false,
        "option": true,
        "contract": true,
        "linear": true,
        "inverse": false,
        "taker": 0.0003,
        "maker": 0.0003,
        "contractSize": 1,
        "expiry": 0,
        "strike": 117000,
        "optionType": "put",
        "precision": {
            "amount": 0.001,
            "price": 0.1
        },
        "limits": {
            "leverage": {},
            "amount": {
                "max": 2
            },
            "price": {},
            "cost": {
                "min": 100
            }
        },
        "info": {
            "symbol": "BTC-USD-117000-P",
            "base_currency": "BTC",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "0.001",
            "price_tick_size": "0.1",
            "min_notional": "100",
            "open_at": 1746692122210,
            "expiry_at": 0,
            "asset_kind": "PERP_OPTION",
            "market_kind": "cross",
            "position_limit": "30",
            "price_bands_width": "0.05",
            "iv_bands_width": "0.05",
            "max_open_orders": 150,
            "max_funding_rate": "0",
            "option_cross_margin_params": {
                "imf": {
                    "long_itm": "0.05",
                    "short_itm": "0.075",
                    "short_otm": "0.05",
                    "short_put_cap": "0.5",
                    "premium_multiplier": "1"
                },
                "mmf": {
                    "long_itm": "0.025",
                    "short_itm": "0.0375",
                    "short_otm": "0.025",
                    "short_put_cap": "0.5",
                    "premium_multiplier": "0.5"
                }
            },
            "price_feed_id": "",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "2",
            "max_funding_rate_change": "0",
            "max_tob_spread": "2",
            "interest_rate": "0",
            "clamp_rate": "0",
            "option_type": "PUT",
            "strike_price": "117000",
            "funding_period_hours": 24,
            "tags": []
        }
    },
    "ADA/USD:USDC": {
        "id": "ADA-USD-PERP",
        "lowercaseId": null,
        "symbol": "ADA/USD:USDC",
        "base": "ADA",
        "quote": "USD",
        "settle": "USDC",
        "baseId": "ADA",
        "quoteId": "USD",
        "settleId": "USDC",
        "type": "swap",
        "spot": false,
        "margin": null,
        "swap": true,
        "future": false,
        "option": false,
        "index": null,
        "active": null,
        "contract": true,
        "linear": true,
        "inverse": false,
        "subType": "linear",
        "taker": 0.0003,
        "maker": -0.00005,
        "contractSize": 1,
        "expiry": null,
        "expiryDatetime": null,
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 1,
            "price": 0.000001
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": null,
                "max": 2100000
            },
            "price": {
                "min": null,
                "max": null
            },
            "cost": {
                "min": 50,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": null,
        "info": {
            "symbol": "ADA-USD-PERP",
            "base_currency": "ADA",
            "quote_currency": "USD",
            "settlement_currency": "USDC",
            "order_size_increment": "1",
            "price_tick_size": "0.000001",
            "min_notional": "50",
            "open_at": "1715925600000",
            "expiry_at": "0",
            "asset_kind": "PERP",
            "market_kind": "cross",
            "position_limit": "6500000",
            "price_bands_width": "0.1",
            "max_open_orders": "75",
            "max_funding_rate": "0.05",
            "delta1_cross_margin_params": {
                "imf_base": "0.05",
                "imf_shift": "0",
                "imf_factor": "0",
                "mmf_factor": "0.5"
            },
            "price_feed_id": "3pyn4svBbxJ9Wnn3RVeafyLWfzie6yC5eTig2S62v9SC",
            "oracle_ewma_factor": "0.20000046249626113",
            "max_order_size": "2100000",
            "max_funding_rate_change": "0.0005",
            "max_tob_spread": "0.02",
            "interest_rate": "0.0001",
            "clamp_rate": "0.0005",
            "funding_period_hours": "8",
            "tags": [
                "LAYER-1"
            ]
        },
        "tierBased": null,
        "percentage": null
    }
}
```

## High-Level Overview

This is a JSON file located at `ts/src/test/static/markets/paradex.json`.



## Detailed Walkthrough

### Code Structure

- Total lines: 503
- Code lines: 503
- Comment lines: 0
- Blank lines: 0

### Main Components



## Usage Examples

No explicit usage examples found in the file. Refer to related test files or documentation.



## Performance & Security Notes

No specific performance or security issues detected.



## Related Files

No explicit file references found.



## Testing & Execution

This appears to be a test file.

**To run this test:**
