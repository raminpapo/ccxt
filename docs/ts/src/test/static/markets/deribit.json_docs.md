# Documentation: ts/src/test/static/markets/deribit.json

## File Metadata

- **Path**: `ts/src/test/static/markets/deribit.json`
- **Size**: 16,717 bytes
- **Lines**: 580
- **Type**: JSON
- **Extension**: .json


## Original Source Code

```json
{
    "BTC/USDT": {
        "id": "BTC_USDT",
        "lowercaseId": null,
        "symbol": "BTC/USDT",
        "base": "BTC",
        "quote": "USDT",
        "settle": null,
        "baseId": "BTC",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": false,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0,
        "maker": 0,
        "contractSize": 0.0001,
        "expiry": 32503708800000,
        "expiryDatetime": "3000-01-01T08:00:00.000Z",
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.0001,
            "price": 1
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 0.0001,
                "max": null
            },
            "price": {
                "min": 1,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1702374650000,
        "info": {
            "price_index": "btc_usdt",
            "kind": "spot",
            "instrument_name": "BTC_USDT",
            "maker_commission": "0.0",
            "taker_commission": "0.0",
            "instrument_type": "linear",
            "expiration_timestamp": "32503708800000",
            "creation_timestamp": "1702374650000",
            "is_active": true,
            "contract_size": "0.0001",
            "tick_size": "1.0",
            "instrument_id": "282196",
            "min_trade_amount": "0.0001",
            "block_trade_commission": "0.0",
            "block_trade_min_trade_amount": "1",
            "block_trade_tick_size": "1.0",
            "base_currency": "BTC",
            "counter_currency": "USDT",
            "quote_currency": "USDT",
            "tick_size_steps": []
        },
        "tierBased": null,
        "percentage": null
    },
    "BTC/USDT:USDT": {
        "id": "BTC_USDT-PERPETUAL",
        "lowercaseId": null,
        "symbol": "BTC/USDT:USDT",
        "base": "BTC",
        "quote": "USDT",
        "settle": "USDT",
        "baseId": "BTC",
        "quoteId": "USDT",
        "settleId": "USDT",
        "type": "swap",
        "spot": false,
        "margin": false,
        "swap": true,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": true,
        "linear": true,
        "inverse": false,
        "subType": "linear",
        "taker": 0.0005,
        "maker": 0,
        "contractSize": 0.001,
        "expiry": 32503708800000,
        "expiryDatetime": "3000-01-01T08:00:00.000Z",
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.001,
            "price": 1
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 0.001,
                "max": null
            },
            "price": {
                "min": 1,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1702464293000,
        "info": {
            "price_index": "btc_usdt",
            "rfq": false,
            "kind": "future",
            "min_trade_amount": "0.001",
            "instrument_name": "BTC_USDT-PERPETUAL",
            "maker_commission": "0.0",
            "taker_commission": "0.0005",
            "instrument_type": "linear",
            "expiration_timestamp": "32503708800000",
            "creation_timestamp": "1702464293000",
            "is_active": true,
            "tick_size": "1.0",
            "contract_size": "0.001",
            "instrument_id": "282352",
            "settlement_period": "perpetual",
            "future_type": "linear",
            "max_leverage": "50",
            "max_liquidation_commission": "0.0075",
            "block_trade_commission": "0.0001",
            "block_trade_min_trade_amount": "200000",
            "block_trade_tick_size": "1.0",
            "settlement_currency": "USDT",
            "base_currency": "BTC",
            "counter_currency": "USDT",
            "quote_currency": "USDT",
            "tick_size_steps": []
        },
        "tierBased": null,
        "percentage": null
    },
    "BTC/USD:BTC-240126-39000-C": {
        "id": "BTC-26JAN24-39000-C",
        "symbol": "BTC/USD:BTC-240126-39000-C",
        "base": "BTC",
        "quote": "USD",
        "settle": "BTC",
        "baseId": "BTC",
        "quoteId": "USD",
        "settleId": "BTC",
        "type": "option",
        "spot": false,
        "margin": false,
        "swap": false,
        "future": false,
        "option": true,
        "active": true,
        "contract": true,
        "linear": false,
        "inverse": true,
        "subType": "inverse",
        "taker": 0.0005,
        "maker": 0.0005,
        "contractSize": 1,
        "expiry": 1706256000000,
        "expiryDatetime": "2024-01-26T08:00:00.000Z",
        "precision": {
            "amount": 0.1,
            "price": 0.0001
        },
        "limits": {
            "leverage": {},
            "amount": {
                "min": 0.1
            },
            "price": {
                "min": 0.0001
            },
            "cost": {}
        },
        "created": 1698220814000,
        "strike": 39000,
        "optionType": "call",
        "info": {
            "tick_size_steps": [
                {
                    "above_price": 0.005,
                    "tick_size": 0.0005
                }
            ],
            "quote_currency": "BTC",
            "min_trade_amount": 0.1,
            "expiration_timestamp": 1706256000000,
            "counter_currency": "USD",
            "settlement_period": "month",
            "settlement_currency": "BTC",
            "block_trade_tick_size": 0.0001,
            "block_trade_min_trade_amount": 0.1,
            "block_trade_commission": 0.00015,
            "option_type": "call",
            "creation_timestamp": 1698220814000,
            "instrument_id": 191504,
            "base_currency": "BTC",
            "contract_size": 1,
            "tick_size": 0.0001,
            "instrument_type": "reversed",
            "taker_commission": 0.0005,
            "maker_commission": 0.0005,
            "strike": 39000,
            "is_active": true,
            "instrument_name": "BTC-26JAN24-39000-C",
            "kind": "option",
            "rfq": false,
            "price_index": "btc_usd"
        }
    },
    "ETH/USDT": {
        "id": "ETH_USDT",
        "lowercaseId": null,
        "symbol": "ETH/USDT",
        "base": "ETH",
        "quote": "USDT",
        "settle": null,
        "baseId": "ETH",
        "quoteId": "USDT",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": false,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0,
        "maker": 0,
        "contractSize": 0.0001,
        "expiry": 32503708800000,
        "expiryDatetime": "3000-01-01T08:00:00.000Z",
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.0001,
            "price": 0.1
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 0.0001,
                "max": null
            },
            "price": {
                "min": 0.1,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1702374671000,
        "info": {
            "price_index": "eth_usdt",
            "kind": "spot",
            "instrument_name": "ETH_USDT",
            "maker_commission": "0.0",
            "taker_commission": "0.0",
            "instrument_type": "linear",
            "expiration_timestamp": "32503708800000",
            "creation_timestamp": "1702374671000",
            "is_active": true,
            "contract_size": "0.0001",
            "tick_size": "0.1",
            "instrument_id": "282197",
            "min_trade_amount": "0.0001",
            "block_trade_commission": "0.0",
            "block_trade_min_trade_amount": "50",
            "block_trade_tick_size": "0.1",
            "base_currency": "ETH",
            "counter_currency": "USDT",
            "quote_currency": "USDT",
            "tick_size_steps": []
        },
        "tierBased": null,
        "percentage": null
    },
    "ETH/USDT:USDT": {
        "id": "ETH_USDT-PERPETUAL",
        "lowercaseId": null,
        "symbol": "ETH/USDT:USDT",
        "base": "ETH",
        "quote": "USDT",
        "settle": "USDT",
        "baseId": "ETH",
        "quoteId": "USDT",
        "settleId": "USDT",
        "type": "swap",
        "spot": false,
        "margin": false,
        "swap": true,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": true,
        "linear": true,
        "inverse": false,
        "subType": "linear",
        "taker": 0.0005,
        "maker": 0,
        "contractSize": 0.01,
        "expiry": 32503708800000,
        "expiryDatetime": "3000-01-01T08:00:00.000Z",
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.01,
            "price": 0.1
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 0.01,
                "max": null
            },
            "price": {
                "min": 0.1,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1702464293000,
        "info": {
            "price_index": "eth_usdt",
            "rfq": false,
            "kind": "future",
            "min_trade_amount": "0.01",
            "instrument_name": "ETH_USDT-PERPETUAL",
            "maker_commission": "0.0",
            "taker_commission": "0.0005",
            "instrument_type": "linear",
            "expiration_timestamp": "32503708800000",
            "creation_timestamp": "1702464293000",
            "is_active": true,
            "tick_size": "0.1",
            "contract_size": "0.01",
            "instrument_id": "282353",
            "settlement_period": "perpetual",
            "future_type": "linear",
            "max_leverage": "50",
            "max_liquidation_commission": "0.0075",
            "block_trade_commission": "0.0001",
            "block_trade_min_trade_amount": "200000",
            "block_trade_tick_size": "0.1",
            "settlement_currency": "USDT",
            "base_currency": "ETH",
            "counter_currency": "USDT",
            "quote_currency": "USDT",
            "tick_size_steps": []
        },
        "tierBased": null,
        "percentage": null
    },
    "SOL/USDC": {
        "id": "SOL_USDC",
        "lowercaseId": null,
        "symbol": "SOL/USDC",
        "base": "SOL",
        "quote": "USDC",
        "settle": null,
        "baseId": "SOL",
        "quoteId": "USDC",
        "settleId": null,
        "type": "spot",
        "spot": true,
        "margin": false,
        "swap": false,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": false,
        "linear": null,
        "inverse": null,
        "subType": null,
        "taker": 0,
        "maker": 0,
        "contractSize": 0.1,
        "expiry": 32503708800000,
        "expiryDatetime": "3000-01-01T08:00:00.000Z",
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 0.1,
            "price": 0.01
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 0.1,
                "max": null
            },
            "price": {
                "min": 0.01,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1709035215000,
        "info": {
            "price_index": "sol_usdc",
            "kind": "spot",
            "instrument_name": "SOL_USDC",
            "maker_commission": "0.0",
            "taker_commission": "0.0",
            "instrument_type": "linear",
            "expiration_timestamp": "32503708800000",
            "creation_timestamp": "1709035215000",
            "is_active": true,
            "contract_size": "0.1",
            "tick_size": "0.01",
            "instrument_id": "296588",
            "min_trade_amount": "0.1",
            "block_trade_commission": "0.0",
            "block_trade_min_trade_amount": "1000",
            "block_trade_tick_size": "0.01",
            "base_currency": "SOL",
            "counter_currency": "USDC",
            "quote_currency": "USDC",
            "tick_size_steps": []
        },
        "tierBased": null,
        "percentage": null
    },
    "BTC/USD:BTC": {
        "id": "BTC-PERPETUAL",
        "lowercaseId": null,
        "symbol": "BTC/USD:BTC",
        "base": "BTC",
        "quote": "USD",
        "settle": "BTC",
        "baseId": "BTC",
        "quoteId": "USD",
        "settleId": "BTC",
        "type": "swap",
        "spot": false,
        "margin": false,
        "swap": true,
        "future": false,
        "option": false,
        "index": null,
        "active": true,
        "contract": true,
        "linear": false,
        "inverse": true,
        "subType": "inverse",
        "taker": 0.0005,
        "maker": 0,
        "contractSize": 10,
        "expiry": 32503708800000,
        "expiryDatetime": "3000-01-01T08:00:00.000Z",
        "strike": null,
        "optionType": null,
        "precision": {
            "amount": 10,
            "price": 0.5
        },
        "limits": {
            "leverage": {
                "min": null,
                "max": null
            },
            "amount": {
                "min": 10,
                "max": null
            },
            "price": {
                "min": 0.5,
                "max": null
            },
            "cost": {
                "min": null,
                "max": null
            }
        },
        "marginModes": {
            "cross": null,
            "isolated": null
        },
        "created": 1534242287000,
        "info": {
            "price_index": "btc_usd",
            "kind": "future",
            "instrument_name": "BTC-PERPETUAL",
            "maker_commission": "0.0",
            "taker_commission": "0.0005",
            "instrument_type": "reversed",
            "expiration_timestamp": "32503708800000",
            "creation_timestamp": "1534242287000",
            "is_active": true,
            "contract_size": "10.0",
            "tick_size": "0.5",
            "instrument_id": "210838",
            "min_trade_amount": "10.0",
            "future_type": "reversed",
            "max_leverage": "50",
            "max_liquidation_commission": "0.0075",
            "block_trade_commission": "2.5e-4",
            "block_trade_min_trade_amount": "200000",
            "block_trade_tick_size": "0.01",
            "settlement_currency": "BTC",
            "settlement_period": "perpetual",
            "base_currency": "BTC",
            "counter_currency": "USD",
            "quote_currency": "USD",
            "tick_size_steps": []
        },
        "tierBased": null,
        "percentage": null
    }
}
```

## High-Level Overview

This is a JSON file located at `ts/src/test/static/markets/deribit.json`.



## Detailed Walkthrough

### Code Structure

- Total lines: 580
- Code lines: 580
- Comment lines: 0
- Blank lines: 0

### Main Components



## Usage Examples

No explicit usage examples found in the file. Refer to related test files or documentation.



## Performance & Security Notes

No specific performance or security issues detected.



## Related Files

No explicit file references found.



## Testing & Execution

This appears to be a test file.

**To run this test:**
