# Documentation: php/async/ascendex.php

## File Metadata

- **Path**: `php/async/ascendex.php`
- **Size**: 186,491 bytes
- **Lines**: 3,771
- **Type**: PHP
- **Extension**: .php
...
class ascendex extends Exchange {
    ...
    public function fetch_markets($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            list($spotMarkets, $contractMarkets) = Async\await(Promise\all(array( $spotPromise, $contractPromise )));
    ...
    public function fetch_spot_markets($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            ...
            //     }
            //
            list($products, $cash) = Async\await(Promise\all(array( $productsPromise, $cashPromise )));
    ...
    public function fetch_contract_markets($params = array ()): PromiseInterface {
        return Async\async(function () use ($params) {
            $contracts = Async\await($this->v2PublicGetFuturesContract ($params));
            //
            //    {
            //        "code" => 0,
            //        "data" => array(
            ...
            //    }
            //
            $data = $this->safe_list($contracts, 'data', array());
            $result = array();
            for ($i = 0; $i < count($data); $i++) {
                $market = $data[$i];
                ...
                $result[] = array(
                    ...
                    'future' => false,
                    'option' => false,
                    'active' => $this->safe_string($market, 'status') === 'Normal',
                    'contract' => true,
                    'linear' => $linear,
                    'inverse' => $inverse,
                    'taker' => $fee,
                    ...
                    'optionType' => null,
                    'precision' => array(
                        'amount' => $this->safe_number($lotSizeFilter, 'lotSize'),
...
This is a PHP file located at `php/async/ascendex.php`.

**Classes defined**: ascendex

**Functions defined**: describe, fetch_accounts, get_account, fetch_markets, parse_balance, fetch_time, fetch_spot_markets, fetch_contract_markets, fetch_currencies, parse_margin_balance
