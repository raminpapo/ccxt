# Documentation: php/async/abstract/bitbns.php

## File Metadata

- **Path**: `php/async/abstract/bitbns.php`
- **Size**: 12,812 bytes
- **Lines**: 227
- **Type**: PHP
- **Extension**: .php


## Original Source Code

```php
<?php

namespace ccxt\async\abstract;

// PLEASE DO NOT EDIT THIS FILE, IT IS GENERATED AND WILL BE OVERWRITTEN:
// https://github.com/ccxt/ccxt/blob/master/CONTRIBUTING.md#how-to-contribute-code


abstract class bitbns extends \ccxt\async\Exchange {
    public function www_get_order_fetchmarkets($params = array()) {
        return $this->request('order/fetchMarkets', 'www', 'GET', $params, null, null, array());
    }
    public function www_get_order_fetchtickers($params = array()) {
        return $this->request('order/fetchTickers', 'www', 'GET', $params, null, null, array());
    }
    public function www_get_order_fetchorderbook($params = array()) {
        return $this->request('order/fetchOrderbook', 'www', 'GET', $params, null, null, array());
    }
    public function www_get_order_gettickerwithvolume($params = array()) {
        return $this->request('order/getTickerWithVolume', 'www', 'GET', $params, null, null, array());
    }
    public function www_get_exchangedata_ohlc($params = array()) {
        return $this->request('exchangeData/ohlc', 'www', 'GET', $params, null, null, array());
    }
    public function www_get_exchangedata_orderbook($params = array()) {
        return $this->request('exchangeData/orderBook', 'www', 'GET', $params, null, null, array());
    }
    public function www_get_exchangedata_tradedetails($params = array()) {
        return $this->request('exchangeData/tradedetails', 'www', 'GET', $params, null, null, array());
    }
    public function v1_get_platform_status($params = array()) {
        return $this->request('platform/status', 'v1', 'GET', $params, null, null, array());
    }
    public function v1_get_tickers($params = array()) {
        return $this->request('tickers', 'v1', 'GET', $params, null, null, array());
    }
    public function v1_get_orderbook_sell_symbol($params = array()) {
        return $this->request('orderbook/sell/{symbol}', 'v1', 'GET', $params, null, null, array());
    }
    public function v1_get_orderbook_buy_symbol($params = array()) {
        return $this->request('orderbook/buy/{symbol}', 'v1', 'GET', $params, null, null, array());
    }
    public function v1_post_currentcoinbalance_everything($params = array()) {
        return $this->request('currentCoinBalance/EVERYTHING', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_getapiusagestatus_usage($params = array()) {
        return $this->request('getApiUsageStatus/USAGE', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_getordersockettoken_usage($params = array()) {
        return $this->request('getOrderSocketToken/USAGE', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_currentcoinbalance_symbol($params = array()) {
        return $this->request('currentCoinBalance/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_orderstatus_symbol($params = array()) {
        return $this->request('orderStatus/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_deposithistory_symbol($params = array()) {
        return $this->request('depositHistory/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_withdrawhistory_symbol($params = array()) {
        return $this->request('withdrawHistory/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_withdrawhistoryall_symbol($params = array()) {
        return $this->request('withdrawHistoryAll/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_deposithistoryall_symbol($params = array()) {
        return $this->request('depositHistoryAll/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_listopenorders_symbol($params = array()) {
        return $this->request('listOpenOrders/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_listopenstoporders_symbol($params = array()) {
        return $this->request('listOpenStopOrders/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_getcoinaddress_symbol($params = array()) {
        return $this->request('getCoinAddress/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_placesellorder_symbol($params = array()) {
        return $this->request('placeSellOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_placebuyorder_symbol($params = array()) {
        return $this->request('placeBuyOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_buystoploss_symbol($params = array()) {
        return $this->request('buyStopLoss/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_sellstoploss_symbol($params = array()) {
        return $this->request('sellStopLoss/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_cancelorder_symbol($params = array()) {
        return $this->request('cancelOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_cancelstoplossorder_symbol($params = array()) {
        return $this->request('cancelStopLossOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_listexecutedorders_symbol($params = array()) {
        return $this->request('listExecutedOrders/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_placemarketorder_symbol($params = array()) {
        return $this->request('placeMarketOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1_post_placemarketorderqnty_symbol($params = array()) {
        return $this->request('placeMarketOrderQnty/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v2_post_orders($params = array()) {
        return $this->request('orders', 'v2', 'POST', $params, null, null, array());
    }
    public function v2_post_cancel($params = array()) {
        return $this->request('cancel', 'v2', 'POST', $params, null, null, array());
    }
    public function v2_post_getordersnew($params = array()) {
        return $this->request('getordersnew', 'v2', 'POST', $params, null, null, array());
    }
    public function v2_post_marginorders($params = array()) {
        return $this->request('marginOrders', 'v2', 'POST', $params, null, null, array());
    }
    public function wwwGetOrderFetchMarkets($params = array()) {
        return $this->request('order/fetchMarkets', 'www', 'GET', $params, null, null, array());
    }
    public function wwwGetOrderFetchTickers($params = array()) {
        return $this->request('order/fetchTickers', 'www', 'GET', $params, null, null, array());
    }
    public function wwwGetOrderFetchOrderbook($params = array()) {
        return $this->request('order/fetchOrderbook', 'www', 'GET', $params, null, null, array());
    }
    public function wwwGetOrderGetTickerWithVolume($params = array()) {
        return $this->request('order/getTickerWithVolume', 'www', 'GET', $params, null, null, array());
    }
    public function wwwGetExchangeDataOhlc($params = array()) {
        return $this->request('exchangeData/ohlc', 'www', 'GET', $params, null, null, array());
    }
    public function wwwGetExchangeDataOrderBook($params = array()) {
        return $this->request('exchangeData/orderBook', 'www', 'GET', $params, null, null, array());
    }
    public function wwwGetExchangeDataTradedetails($params = array()) {
        return $this->request('exchangeData/tradedetails', 'www', 'GET', $params, null, null, array());
    }
    public function v1GetPlatformStatus($params = array()) {
        return $this->request('platform/status', 'v1', 'GET', $params, null, null, array());
    }
    public function v1GetTickers($params = array()) {
        return $this->request('tickers', 'v1', 'GET', $params, null, null, array());
    }
    public function v1GetOrderbookSellSymbol($params = array()) {
        return $this->request('orderbook/sell/{symbol}', 'v1', 'GET', $params, null, null, array());
    }
    public function v1GetOrderbookBuySymbol($params = array()) {
        return $this->request('orderbook/buy/{symbol}', 'v1', 'GET', $params, null, null, array());
    }
    public function v1PostCurrentCoinBalanceEVERYTHING($params = array()) {
        return $this->request('currentCoinBalance/EVERYTHING', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostGetApiUsageStatusUSAGE($params = array()) {
        return $this->request('getApiUsageStatus/USAGE', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostGetOrderSocketTokenUSAGE($params = array()) {
        return $this->request('getOrderSocketToken/USAGE', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostCurrentCoinBalanceSymbol($params = array()) {
        return $this->request('currentCoinBalance/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostOrderStatusSymbol($params = array()) {
        return $this->request('orderStatus/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostDepositHistorySymbol($params = array()) {
        return $this->request('depositHistory/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostWithdrawHistorySymbol($params = array()) {
        return $this->request('withdrawHistory/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostWithdrawHistoryAllSymbol($params = array()) {
        return $this->request('withdrawHistoryAll/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostDepositHistoryAllSymbol($params = array()) {
        return $this->request('depositHistoryAll/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostListOpenOrdersSymbol($params = array()) {
        return $this->request('listOpenOrders/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostListOpenStopOrdersSymbol($params = array()) {
        return $this->request('listOpenStopOrders/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostGetCoinAddressSymbol($params = array()) {
        return $this->request('getCoinAddress/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostPlaceSellOrderSymbol($params = array()) {
        return $this->request('placeSellOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostPlaceBuyOrderSymbol($params = array()) {
        return $this->request('placeBuyOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostBuyStopLossSymbol($params = array()) {
        return $this->request('buyStopLoss/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostSellStopLossSymbol($params = array()) {
        return $this->request('sellStopLoss/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostCancelOrderSymbol($params = array()) {
        return $this->request('cancelOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostCancelStopLossOrderSymbol($params = array()) {
        return $this->request('cancelStopLossOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostListExecutedOrdersSymbol($params = array()) {
        return $this->request('listExecutedOrders/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostPlaceMarketOrderSymbol($params = array()) {
        return $this->request('placeMarketOrder/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v1PostPlaceMarketOrderQntySymbol($params = array()) {
        return $this->request('placeMarketOrderQnty/{symbol}', 'v1', 'POST', $params, null, null, array());
    }
    public function v2PostOrders($params = array()) {
        return $this->request('orders', 'v2', 'POST', $params, null, null, array());
    }
    public function v2PostCancel($params = array()) {
        return $this->request('cancel', 'v2', 'POST', $params, null, null, array());
    }
    public function v2PostGetordersnew($params = array()) {
        return $this->request('getordersnew', 'v2', 'POST', $params, null, null, array());
    }
    public function v2PostMarginOrders($params = array()) {
        return $this->request('marginOrders', 'v2', 'POST', $params, null, null, array());
    }
}

```

## High-Level Overview

This is a PHP file located at `php/async/abstract/bitbns.php`.

**Classes defined**: bitbns

**Functions defined**: www_get_exchangedata_tradedetails, www_get_order_fetchorderbook, v1_get_orderbook_sell_symbol, www_get_order_gettickerwithvolume, www_get_exchangedata_ohlc, www_get_order_fetchmarkets, www_get_exchangedata_orderbook, v1_get_platform_status, v1_get_tickers, www_get_order_fetchtickers



## Detailed Walkthrough

### Code Structure

- Total lines: 227
- Code lines: 220
- Comment lines: 2
- Blank lines: 5

### Main Components

**Functions** (72):
- `v1GetOrderbookBuySymbol()`
- `v1GetOrderbookSellSymbol()`
- `v1GetPlatformStatus()`
- `v1GetTickers()`
- `v1PostBuyStopLossSymbol()`
- `v1PostCancelOrderSymbol()`
- `v1PostCancelStopLossOrderSymbol()`
- `v1PostCurrentCoinBalanceEVERYTHING()`
- `v1PostCurrentCoinBalanceSymbol()`
- `v1PostDepositHistoryAllSymbol()`
- `v1PostDepositHistorySymbol()`
- `v1PostGetApiUsageStatusUSAGE()`
- `v1PostGetCoinAddressSymbol()`
- `v1PostGetOrderSocketTokenUSAGE()`
- `v1PostListExecutedOrdersSymbol()`
- `v1PostListOpenOrdersSymbol()`
- `v1PostListOpenStopOrdersSymbol()`
- `v1PostOrderStatusSymbol()`
- `v1PostPlaceBuyOrderSymbol()`
- `v1PostPlaceMarketOrderQntySymbol()`



## Usage Examples

No explicit usage examples found in the file. Refer to related test files or documentation.



## Performance & Security Notes

No specific performance or security issues detected.



## Related Files

No explicit file references found.



## Testing & Execution

**To execute this PHP file:**

```bash
php php/async/abstract/bitbns.php
```

